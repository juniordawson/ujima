...
function station_shortcode_dj_on_air($atts) {
	...
	$dj_str .= '<div class="on-air-list">';

	//echo the show/dj currently on-air
	if($djs['type'] == 'override') {

		$dj_str .= '<div class="on-air-dj">';
		if($show_avatar) {
		...
		if(isset($djs['all']) && count($djs['all']) > 0) {
			foreach($djs['all'] as $dj) {
				...
				$dj_str .= '<h2 class="on-air-dj-title mb-1">';
				if($show_link) {
					$dj_str .= '<a href="';
				...
				if($show_playlist) {
					$dj_str .= '<p class="on-air-dj-playlist"><a href="'.$playlist['playlist_permalink'].'">'.__('View show info »', 'radio-station').'</a></p>';
				}
				...
				if($show_sched) {
					...
					if(!$show_all_sched) { //if we only want the schedule that's relevant now to display...
						...
						if($current_sched) {
							if($time == 12) {
								$dj_str .= '<p class="on-air-dj-sched">'.__($current_sched['day'], 'radio-station').', '.$current_sched['start_hour'].':'.$current_sched['start_min'].' '.$current_sched['start_meridian'].'-'.$current_sched['end_hour'].':'.$current_sched['end_min'].' '.$current_sched['end_meridian'].'</p>';
							}
							else {
								$current_sched = station_convert_schedule_to_24hour($current_sched);
					
								$dj_str .= '<p class="on-air-dj-sched">'.__($current_sched['day'], 'radio-station').', '.$current_sched['start_hour'].':'.$current_sched['start_min'].' '.'-'.$current_sched['end_hour'].':'.$current_sched['end_min'].'</p>';
							}
						}
						...
						foreach($scheds as $sched) {
							if($time == 12) {
								$dj_str .= '<p class="on-air-dj-sched">'.__($sched['day'], 'radio-station').', '.$sched['start_hour'].':'.$sched['start_min'].' '.$sched['start_meridian'].'-'.$sched['end_hour'].':'.$sched['end_min'].' '.$sched['end_meridian'].'</p>';
							}
							else {
								$sched = station_convert_schedule_to_24hour($sched);
									
								$dj_str .= '<p class="on-air-dj-sched">'.__($sched['day'], 'radio-station').', '.$sched['start_hour'].':'.$sched['start_min'].' '.'-'.$sched['end_hour'].':'.$sched['end_min'].'</p>';
							}
						}
					}
				}
			...
			$dj_str .= '<div class="on-air-dj default-dj">'.$default_name.'</div>';
		}
	}
